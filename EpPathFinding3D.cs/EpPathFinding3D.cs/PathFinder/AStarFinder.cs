        ...
        public static List<GridPos> FindPath(AStarParam iParam)
        {
            ...
            while (openList.Count != 0)
            {
                ...
                //Parallel.ForEach(neighbors, neighbor =>
                {
                    ...
                    float ng = node.startToCurNodeLen + (float)((x - node.x == 0 || y - node.y == 0) ? (z - node.z == 0) ? 1:Math.Sqrt(2) : (z-node.z == 0) ? Math.Sqrt(2):Math.Sqrt(3));

                    if (!neighbor.isOpened || ng < neighbor.startToCurNodeLen)
                    {
                        neighbor.startToCurNodeLen = ng;
                        if (neighbor.heuristicCurNodeToEndLen == null) neighbor.heuristicCurNodeToEndLen = weight * heuristic(Math.Abs(x - endNode.x), Math.Abs(y - endNode.y), Math.Abs(z - endNode.z));
